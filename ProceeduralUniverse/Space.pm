...
sub DetermineOrbits {
	...
	while ($current_orbit > $args->{STAR}->{INNERLIMIT}){
		unless(defined $args->{STAR}->{FORBIDDENZONEOUTSIDERADIUS} && $current_orbit < $args->{STAR}->{FORBIDDENZONEOUTSIDERADIUS} && defined $args->{STAR}->{FORBIDDENZONEINSIDERADIUS} && $current_orbit > $args->{STAR}->{FORBIDDENZONEINSIDERADIUS}){
			push(@orbits, {ORBITALRADIUS => $current_orbit});
		}
		$current_orbit = $current_orbit / $orbitalspacing;
		$orbitalspacing = DetermineOrbitalSpacing();
	}
